#在测试数据集，使用loss，按照每个步进行输出；
#验证数据集，使用precision, recall, f1等指标，按照epoch进行输出
#使用tensorboard ，没有不要安装tesorboardX
...
def eval():
    ...
    for i, batch in enumerate(tqdm(train_loader)):
        ...
        precision, recall, f1, _ = precision_recall_fscore_support(label.cpu(), predict.cpu(), average='binary')
        acc = accuracy_score(label.cpu(), predict.cpu())
        print("precision, recall, f1, = ",precision, recall, f1)
        print("acc = ",acc)
    writer.add_scalar('acc', acc, (i + 1))
    writer.add_scalar('precision', precision, (i + 1))
    writer.add_scalar('f1', f1, (i + 1))
    writer.add_scalar('Eval_loss', torch.mean(loss, dim=0), (i + 1))
